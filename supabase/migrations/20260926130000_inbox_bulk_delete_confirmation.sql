-- =============================================================================
-- Hlasové hromadné zmazanie nepriradených dokladov z Inboxu — potvrdenie.
--
-- Aplikované po výslovnom schválení cez Supabase MCP apply_migration
-- (supabase/MIGRATIONS.md). NIKDY db push.
--
-- ČO A PREČO
-- ----------
-- Intent INBOX_DELETE_UNASSIGNED ide cez ten istý jednorazový HMAC
-- potvrdzovací tok ako ostatné zápisy. Tabuľka aj funkcia majú allowlist
-- intentov (CHECK + IF), preto bez tejto zmeny server náhľad s potvrdením
-- nevytvorí (fail closed — nič sa nezmaže).
--
-- Mení sa IBA allowlist. Žiadna tabuľka, stĺpec, RLS politika ani grant sa
-- nerozširuje; funkcia ostáva SECURITY DEFINER so search_path = '' a
-- schémou kvalifikovanými názvami, EXECUTE iba pre authenticated.
-- Samotné mazanie dokladov robí používateľ pod vlastnou RLS
-- (documents_delete_finance_manager) — rovnako ako tlačidlo v Inboxe.
-- =============================================================================
